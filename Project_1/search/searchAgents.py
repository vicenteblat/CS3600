...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        return sorted(state[2]) == sorted(self.corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty] and (nextx, nexty) not in visited_corners:
                if (nextx, nexty) in self.corners:
                    new_corner_list = list(visited_corners)
                    new_corner_list.append((nextx, nexty))
                    nextState = (nextx, nexty, tuple(new_corner_list))
                else:
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    corners_set = set(problem.corners)
    visited_corners_set = set(state[2])
    new_corners = tuple(corners_set.difference(visited_corners_set))
    current = (state[0], state[1])
    h = 0
    closest_corner = ()
    corners_list = list(new_corners)
    shortest_distance = math.inf
    while corners_list:
        for corner in corners_list:
            manhattan = abs(current[0] - corner[0]) + abs(current[1] - corner[1])
...
def foodHeuristic(state, problem):
    ...
    for coordinate in food_coordinates:
        manhattan = abs(position[0] - coordinate[0]) + abs(position[1] - coordinate[1])
        if manhattan < shortest_distance:
            shortest_distance = manhattan
    ...
    for coordinate in food_coordinates:
        manhattan = abs(position[0] - coordinate[0]) + abs(position[1] - coordinate[1])
        if manhattan > longest_distance:
            longest_distance = manhattan
    h += longest_distance

    if not any(foodGrid.asList()):
        h = 0
...
class ApproximateSearchAgent(Agent):
    ...
    def getAction(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return search.aStarSearch(AnyFoodSearchProblem(state), foodHeuristic)
